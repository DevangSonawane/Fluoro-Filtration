          ...
          {productCategories.map((category) => (
            <article key={category} className="card card-pad">
              ...
              <div className={styles.cardActions}>
                <Link href={`/products#${categoryToAnchor(category)}`} className="btn btn-secondary">
                  Jump to family grid
                </Link>
                <Link href={`/products/${productsByCategory(category)[0]?.slug}`} className="btn btn-primary">
                  View a family
                </Link>
              </div>
            </article>
